...
export default async function handler(request, response) {
  ...
  const { MAILJET_API_KEY, MAILJET_SECRET_KEY, MAILJET_FROM_EMAIL, MAILJET_TO_EMAIL } = process.env;
  if (![MAILJET_API_KEY, MAILJET_SECRET_KEY, MAILJET_FROM_EMAIL, MAILJET_TO_EMAIL].every(Boolean)) return response.status(503).json({ error: "Contact service unavailable" });
  ...
  try {
    const mailjet = await fetch("https://api.mailjet.com/v3.1/send", {
      ...
      body: JSON.stringify({ Messages: [{ From: { Email: MAILJET_FROM_EMAIL, Name: "KnoXia" }, To: [{ Email: MAILJET_TO_EMAIL }], ReplyTo: { Email: fields[2], Name: name }, Subject: `[KnoXia] ${fields[3]}`, TextPart: text, HTMLPart: `<p><strong>From:</strong> ${escapeHtml(name)} &lt;${escapeHtml(fields[2])}&gt;</p><p><strong>Subject:</strong> ${escapeHtml(fields[3])}</p><p>${escapeHtml(fields[4]).replace(/\n/g, "<br>")}</p>` }] })
    });
